...
`specnaut cloud login` (#353) — or its top-level alias **`specnaut login`** — obtains an access
token and a refresh token for your Specnaut Cloud deployment. Those are secrets; this page explains
where they are kept at rest and how to control it. (To connect from scratch, run
`specnaut init --backlog cloud` then `specnaut login`.)
...
where that URL came from, and asks for confirmation the **first** time you authenticate against a
URL supplied by a project's `.specnaut/backlog-config.yml` (#400) — so a cloned repo can't silently
